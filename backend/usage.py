...
async def get_admin_headers(client: httpx.AsyncClient):
    """
    Authenticates as an admin and returns the Authorization header.
...
async def check_usage_limit(tenant_id: str, limit: int = 100):
    ...
    async with httpx.AsyncClient() as client:
        ...
        try:
            # 👉 NEW: Pass headers into the request
            response = await client.get(url, params=params, headers=headers)
            
            if response.status_code != 200:
                print(f"⚠️ Usage Check PB Error: {response.text}")
                return True, 0
...
async def log_chat(tenant_id: str, user_query: str, ai_response: str):
    ...
    async with httpx.AsyncClient() as client:
        try:
            ...
            # 👉 NEW: Pass headers into the POST request
